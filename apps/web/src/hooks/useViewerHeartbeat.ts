 ...
 *
 * Details (docs/analytics.md): heartbeat is anonymous and needs only a
 * stable viewer id (persisted in localStorage); it never touches the user
 * account. Pings are skipped while the tab is hidden and the interval stops
 * as soon as the stream ends or the page unmounts. A failed ping is
...
export function useViewerHeartbeat(streamId: string | undefined, isLive: boolean) {
  useEffect(() => {
    ...
    const ping = async () => {
      try {
        const result = await analyticsApi.heartbeat(streamId, viewerId);
